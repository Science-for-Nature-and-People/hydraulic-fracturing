...
function createMaterialWordCloud(filterData){
wordData = filterData;

//lop through and count number of words
var objectsDetails = {};
      $.each(wordData, function(i,e){
        objectsDetails[this.material_detail] = (objectsDetails[this.material_detail] || 0) + 1;
      });
...
//    list: tags.map(function(word) { return [word[0], Math.round(word[1])]; }),
    list: tags.map(function(word) { return [word[0], Math.round(word[1]/6+4)]; })
